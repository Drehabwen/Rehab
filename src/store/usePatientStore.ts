  ...
  importSquatLabScreening: async (payload: SyncScreeningPayload, patientId?: string): Promise<Patient> => {
    ...
    try {
      ...
      if (patientId) {
        ...
        // Create new patient
        // If the early screening ID is a valid 4-letter clinic code, reuse it; otherwise generate a new one
        const isValid4Letter = /^[A-Z]{4}$/.test(payload.subject.subject_id);
        const newPatientId = isValid4Letter ? payload.subject.subject_id : generatePatientId();
