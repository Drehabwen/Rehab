...
export function buildSessionInsightCards(sessionInput: SessionReportInput): SessionInsightCard[] {
  ...
  if (posture && rom) {
    cards.push({
      ...
      summary: '客观体态结果和关节活动范围结果已经进入同一次接诊，可在综合报告里同步判断对位异常与活动受限是否一致。',
      evidence: [
        `体态 ${posture.status === 'ready' ? '已就绪' : '部分到位'}`,
        `ROM ${rom.status === 'ready' ? '已就绪' : '部分到位'}`,
      ],
      action: '下一步可以在同一份综合报告中联读姿态异常、代偿链和 ROM 限制，不再分散在两个模块里判断。',
    });
  }
  ...
  if (posture && medvoice) {
    cards.push({
      id: 'posture-voice',
      tone: 'violet',
      title: '症状描述可以和体态证据相互对照',
      summary: '主诉、现病史和体态评估已进入同一接诊上下文，可以在综合报告中判断主观不适与客观姿态异常是否同向。',
      evidence: [
  ...
  if (medvoice && !rom) {
    cards.push({
      ...
      tone: 'amber',
      title: '仍缺少 ROM 证据层',
      summary: '已经有语音病历或体态输入，但缺少关节活动范围数据时，综合报告对功能受限的判断仍会偏经验化。',
      evidence: ['ROM 输入缺失', `当前已到位 ${sessionInput.readiness.readyCount}/3`],
      action: '如果患者主诉涉及疼痛、僵硬或活动受限，建议补做 ROM 评估后再生成综合报告。',
    });
  }
  ...
  if (sessionInput.readiness.readyCount <= 1) {
    cards.push({
      ...
      tone: 'amber',
      title: '综合上下文仍然偏薄',
      summary: '当前进入报告中心的输入不足两类，综合结论容易退化成单模块扩写，不适合作为最终全局报告。',
      evidence: [
        `已就绪 ${sessionInput.readiness.readyCount}/3`,
        `缺失 ${sessionInput.readiness.missingTypes.join(' / ') || '无'}`,
      ],
  ...
  if (cards.length === 0) {
    cards.push({
      ...
      action: '下一步可以从报告中心统一生成综合报告，并将结果衔接到治疗方案。',
    });
  }
...
export function buildSessionDraftReport(sessionInput: SessionReportInput): string {
  const lines: string[] = [
    ...
    '',
    '## 综合编排说明',
    '当前内容是报告中心的编排预览，用来确认 posture、ROM、语音病历是否进入同一次接诊上下文。',
    '后续真正的综合 LLM 报告会从这里发起，并基于这些输入统一生成。',
  ];
