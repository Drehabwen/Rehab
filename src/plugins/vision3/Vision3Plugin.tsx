...
export const Vision3Plugin: React.FC = () => {
  ...
  const handleFinishStepped = useCallback(() => {
    // Collect all results and trigger batch analysis
  ...
  const handleResetToEntry = useCallback(() => {
    ...
    setSteppedResults({});
    setCaptureStatus('idle');
  }, [setSteppedResults, setCaptureStatus]);
  ...
  return (
    <div className="h-full flex flex-col gap-6 animate-in fade-in duration-500 overflow-hidden relative">
      {/* 评估指标边栏 (Right Dashboard) - Phase 4 */}
      {/* 渐进式显露：仅在评估完成后显示详细指标，避免采样阶段干扰 */}
      {step === 'completed' && wsResult && (
          ...
          <div className="bg-white/40 backdrop-blur-3xl p-6 rounded-[2.5rem] border border-white/60 shadow-2xl pointer-events-auto animate-in slide-in-from-right-8 duration-1000 ease-out">
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-white/40 rounded-2xl border border-white/60 group hover:border-antey-primary/30 transition-all duration-500">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className={cn(
                    ...
                    Math.abs(wsResult?.metrics?.swayOffset || 0) < 15 ? "bg-emerald-500/10 text-emerald-600" : "bg-rose-500/10 text-rose-600"
                  )}>
                    {Math.abs(wsResult?.metrics?.swayOffset || 0) < 15 ? 'Excellent' : 'Offset'}
                  </span>
                </div>
                <div className="flex items-end gap-2">
                  <span className="text-3xl font-light text-slate-900 tracking-tighter">{(wsResult?.metrics?.swayOffset || 0).toFixed(1)}</span>
                ...
                <div className="mt-4 w-full h-1 bg-slate-200/50 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${Math.min(100, Math.abs(wsResult?.metrics?.swayOffset || 0) * 2)}%` }}
                  />
                </div>
              </div>
              ...
              <div className="p-4 bg-white/40 rounded-2xl border border-white/60 group hover:border-antey-primary/30 transition-all duration-500">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className={cn(
                    "text-[9px] px-2.5 py-1 rounded-full font-black uppercase tracking-tighter",
                    Math.abs(wsResult?.metrics?.shoulderAngle || 0) < 3 ? "bg-emerald-500/10 text-emerald-600" : "bg-rose-500/10 text-rose-600"
                  )}>
                    {Math.abs(wsResult?.metrics?.shoulderAngle || 0) < 3 ? 'Balanced' : 'Tilted'}
                  </span>
                </div>
                <div className="flex items-end gap-2">
                  <span className="text-3xl font-light text-slate-900 tracking-tighter">{(wsResult?.metrics?.shoulderAngle || 0).toFixed(1)}</span>
              ...
              <div className="p-4 bg-white/40 rounded-2xl border border-white/60 group hover:border-antey-primary/30 transition-all duration-500">
                ...
                <div className="flex items-end gap-2">
                  <span className="text-3xl font-light text-slate-900 tracking-tighter">{(wsResult?.metrics?.hipAngle || 0).toFixed(1)}</span>
                  <span className="text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-widest">°</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-slate-200/60">
              ...
              <p className="text-[11px] text-slate-500 leading-relaxed font-medium">
                {wsResult?.stability && wsResult.stability.sd < 10 
                  ? "数据采集质量极高，已达到临床级分析标准。" 
      ...
      <React.Suspense fallback={
        ...
        {activeTab === 'posture' && isEntryMode ? (
          ...
          <div ref={videoContainerRef} className={cn(
            ...
            {/* Phase 4: 评估交互层 */}
            {/* Overlay: Real-time or Stepped or Result */}
