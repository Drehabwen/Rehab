...
import { Scan, Activity, History, Zap, Dna, Settings2, ArrowRight } from 'lucide-react';

interface EntryHubProps {
  onSelectView: (view: 'front' | 'side' | 'back') => void;
}

export const Vision3EntryHub: React.FC<EntryHubProps> = ({ onSelectView }) => {
  const cards = [
        ...
        <button
          ...
          onClick={() => {
            if (card.id === 'front' || card.id === 'side' || card.id === 'back') {
              onSelectView(card.id as 'front' | 'side' | 'back');
            }
          }}
          className={cn(
